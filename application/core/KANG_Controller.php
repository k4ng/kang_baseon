<?php
	class KANG_Controller extends CI_Controller
	{
			    ...
			    case "header":
					...
					$tags .= '<html>';
					    $tags .= '<head>';
					        ...
					        $tags .= '<!-- App Favicon -->';
					        $tags .= '<link rel="shortcut icon" href="'.$this->_asset_path('configs').'/'.$favicon.'">';
					        ...
					        if(isset($ad_header_css))
					        {
					        	if(count($ad_header_css) != 0)
					        	{
					        		...
					        		while ($ad_cid < count($ad_header_css)) {
					        			$ad_css_loop .= '<link href="'.$this->_asset_path().'/'.$ad_header_css[$ad_cid].'?v=14154930'.$ad_cid.'" rel="stylesheet" type="text/css">';
						        		$ad_cid++;
					        		}
					        		$tags .= $ad_css_loop;
					        	}
					        ...
					        */
					        ...
					        if($param_css != 0)
					        {
						        $cid = 0;
						        while($cid < $param_css) {
						        	$css_loop .= '<link href="'.$this->_asset_path().'/'.$param["css"][$cid].'?v=14154930'.$cid.'" rel="stylesheet" type="text/css">';
						        	$cid++;
						        }
						        $tags .= $css_loop;
						    }
					        ...
					        */
						    ...
						    while($jfsid < $jfs)
						    {
						    	...
						    	if($jf_arr['load'] == TRUE)
						    	{
						    		...
						    		$tags .= '<script src="'.$this->_asset_path('frameworks').'/'.$jfs_name.'/'.$jf_arr['version'].'/'.$jfs_name.'.'.$js_minify.'js'.'?v=14154930'.$jfsid.'"></script>';
						    	}
						    	$jfsid++;
						    }
					        ...
					        if(isset($ad_header_js))
					        {
					        	if(count($ad_header_js) != 0)
					        	{
					        		...
					        		while ($ad_jid < count($ad_header_js)) {
					        			$ad_js_loop .= '<script src="'.$this->_asset_path().'/'.$ad_header_js[$ad_jid].'?v=14154930'.$ad_jid.'"></script>';
						        		$ad_jid++;
					        		}
					        		$tags .= $ad_js_loop;
					        	}
					        ...
					        */
						    ...
						    if($param_js != 0)
					        {
						        $jid = 0;
						        while($jid < $param_js) {
						        	$js_loop .= '<script src="'.$this->_asset_path().'/'.$param["js"][$jid].'?v=14154930'.$jid.'"></script>';
						        	$jid++;
						        }
						        $tags .= $js_loop;
						    }
				        ...
				        if(isset($ad_footer_css))
				        {
				        	if(count($ad_footer_css) != 0)
				        	{
				        		...
				        		while ($ad_cid < count($ad_footer_css)) {
				        			$ad_css_loop .= '<link href="'.$this->_asset_path().'/'.$ad_footer_css[$ad_cid].'?v=14154930'.$ad_cid.'" rel="stylesheet" type="text/css">';
					        		$ad_cid++;
				        		}
				        		$tags .= $ad_css_loop;
				        	}
				        ...
				        if(isset($ad_footer_js))
				        {
				        	if(count($ad_footer_js) != 0)
				        	{
				        		...
				        		while ($ad_jid < count($ad_footer_js)) {
				        			$ad_js_loop .= '<script src="'.$this->_asset_path().'/'.$ad_footer_js[$ad_jid].'?v=14154930'.$ad_jid.'"></script>';
					        		$ad_jid++;
				        		}
				        		$tags .= $ad_js_loop;
				        	}
		 ...
		 */
		function _asset_path( $path = "style_back" ){
			switch ($path) {
				case 'configs':
